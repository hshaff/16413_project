...
RRT Helper Functions:  
    ...
    steer() -- takes a step in the direction of the randomly sampled point from the nearest node in the graph and outputs a new pose to be added to the graph   
...
Motion Planner Helper Functions:  
    ...
    perform_actions() -- when RRT has completed and the arm has been moved to the goal location, the function performs the current action. For example, for 'open-drawer' the function calls the open_surface_joint() function to set the drawer joint to the open configuration.  
...
We chose to implement RRT for our motion planner. Based on the next action for the robot to complete, we generate a goal pose and RRT randomly samples the space to generate a feasible path to the goal. Once close enough to the goal, the action can be completed and the motion planner continues with the next activity generated by the activity planner. To implement collision checking in RRT within the simulation environment, when we randomly generated a new position, we checked to see if there was a feasible joint configuration for the arm to take to get to that position. If there was no feasible configuration, the randomly generated pose was not added to the RRT graph. Therefore, when it was infeasible for the arm to reach the goal, RRT returned an empty path indicating that we needed to move the base of the robot toward the goal in order for RRT to find a valid solution.
